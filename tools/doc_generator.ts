...
export async function generateDocument(tipo: "cotizacion" | "recibo", datos: DocData, sessionId: string): Promise<string> {
  try {
    ...
    const docId = `doc-${timestamp}-${Math.floor(Math.random() * 1000)}`;
    ...
    for (const item of datos.items) {
      ...
      filasHtml += `
        <div class="flex items-center border-b border-gray-100 py-2 px-2 text-[11px] hover:bg-gray-50 transition-colors">
          ...
          <div class="w-[48%] pl-1">
              <div class="font-medium text-gray-800 uppercase">${item.nombre || 'Item'}</div>
              ${item.comentario ? `<div class="text-[9px] text-gray-500 italic mt-0.5">Obs: ${item.comentario}</div>` : ""}
          </div>
          <div class="w-[15%] text-right text-gray-600">S/ ${(unitarioConIGV / 1.18).toFixed(2)}</div>
          <div class="w-[15%] text-right pr-2 font-medium">S/ ${importe.toFixed(2)}</div>
        </div>
      `;
    }
